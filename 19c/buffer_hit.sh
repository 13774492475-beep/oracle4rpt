...
#filename="buffer_hit_${timestamp}.csv"
filename="buffer_hit.csv"
...
physical_reads_direct AS (
    ...
    WHERE 
        s.dbid = (SELECT dbid FROM current_db)
        AND s.instance_number = (SELECT instance_number FROM current_db)
        AND s.stat_name = 'physical reads direct'
),
physical_reads_direct_lob AS (
    SELECT 
        SUM(CASE WHEN s.snap_id = r.end_snap THEN value ELSE 0 END) AS end_value,
        SUM(CASE WHEN s.snap_id = r.beg_snap THEN value ELSE 0 END) AS start_value
    FROM 
    ...
    WHERE 
        s.dbid = (SELECT dbid FROM current_db)
        AND s.instance_number = (SELECT instance_number FROM current_db)
        AND s.stat_name = 'physical reads direct (lob)'
),
session_logical_reads AS (
    SELECT 
        SUM(CASE WHEN s.snap_id = r.end_snap THEN value ELSE 0 END) AS end_value,
        SUM(CASE WHEN s.snap_id = r.beg_snap THEN value ELSE 0 END) AS start_value
    FROM 
...
SELECT 
    ROUND(
        100 * (
            1 - (
                (
                    (SELECT end_value - start_value FROM physical_reads) - 
                    ((SELECT end_value - start_value FROM physical_reads_direct) + 
                    NVL((SELECT end_value - start_value FROM physical_reads_direct_lob), 0))
                ) / 
                (SELECT end_value - start_value FROM session_logical_reads)
            )
        ),
        2
    ) AS "Buffer Hit%"
