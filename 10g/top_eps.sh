...
SELECT *
  FROM (SELECT NVL(sqt.elap / 1000000, TO_NUMBER(NULL)) AS elap_secs,
               NVL(sqt.cput / 1000000, TO_NUMBER(NULL)) AS cput_secs,
               sqt.exec,
               DECODE(sqt.exec, 0, TO_NUMBER(NULL), (sqt.elap / sqt.exec / 1000000)) AS avg_elap_secs,
               (100 * (sqt.elap / db_time.dbtime)) AS norm_val,
               sqt.sql_id,
               TO_CLOB(DECODE(sqt.module, NULL, NULL, 'Module: ' || sqt.module)) AS module_info,
          ...
          CROSS JOIN (SELECT SUM(e.value) - SUM(b.value) AS dbtime
                      ...
                      WHERE b.snap_id = (SELECT MIN(snap_id)
                        ...
                        AND b.stat_name = 'DB time') db_time
