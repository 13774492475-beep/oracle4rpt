...
#filename="exec_to_parse_${timestamp}.csv"
filename="exec_to_parse.csv"
...
parse_execute_stats AS (
    SELECT 
        snap_id,
        SUM(CASE WHEN stat_name = 'parse count (total)' THEN value ELSE 0 END) AS total_parse_count,
        SUM(CASE WHEN stat_name = 'execute count' THEN value ELSE 0 END) AS execute_count
    FROM 
        DBA_HIST_SYSSTAT
    WHERE 
        ...
        AND stat_name IN ('parse count (total)', 'execute count')
    GROUP BY 
        snap_id
),
start_end_values AS (
    ...
    ROUND(
        100 * (
            1 - (
                ...
                NULLIF(
                    (SUM(CASE WHEN snap_id = (SELECT end_snap FROM start_end_values) THEN execute_count ELSE 0 END) - 
                     SUM(CASE WHEN snap_id = (SELECT start_snap FROM start_end_values) THEN execute_count ELSE 0 END)), 
                    0
                )
            )
        ),
        2
    ) AS "Parse to Execute Ratio%"
FROM 
    parse_execute_stats;
